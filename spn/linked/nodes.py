...
class SumNode(Node):
    ...
    def exact_eval(self):
        ...
        w_sum = 0
        for node, log_weight in zip(self.children, self.log_weights):
            w_sum += exp(node.log_val + log_weight)
    ...
    def deduce_p(self):
        self.p = 0
        for child in self.children:
            self.p = max(self.p, child.p)

    def get_big_equation(self):
        equation = "("
        for ith_child in range(self.n_children()):
            child = self.children[ith_child]
            w = self.weights[ith_child]
            equation = equation + "{}*".format(w) + child.get_big_equation() + "+"
        equation = equation[:-1] + ")"
        return equation
...
class ProductNode(Node):
    ...
    def get_big_equation(self):
        equation = ""
        for child in self.children:
            equation = equation + child.get_big_equation() + "*"
        equation = equation[:-1]
        return equation
...
class CategoricalSmoothedNode(Node):
    ...
    def bound_eval(self, input, dic):
        """
        WRITEME
        """
        obs = input[self.var]
        if obs == MAXI:
            self.log_val = max(self._var_probs)
            if self.var in dic :
                if dic[self.var] != numpy.argmax(self._var_probs):
                    dic[self.var] = -1
            else :
                dic[self.var] = numpy.argmax(self._var_probs)
    ...
    def get_big_equation(self):
        equation = '({w0}*(1-model.l[{v}])+{w1}*model.l[{v}])'.format(w0 = exp(self._var_probs[0]),
                                                                      w1 = exp(self._var_probs[1]),
                                                                      v= self.var)
        return equation
