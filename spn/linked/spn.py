...
class Spn(AbstractSpn):
    ...
    def exact_eval(self, input):
        self._input_layer.eval(input)
        for layer in self._layers:
            layer.exact_eval()
        return math.exp(self._root_layer._nodes[0].log_val)

    def bound_eval(self, input):
        dic = self._input_layer.bound_eval(input)
        for layer in self._layers:
            layer.exact_eval()

        evidence = list(dic.values())
        if -1 in evidence:
            return math.exp(self._root_layer._nodes[0].log_val), False, evidence
        else :
            return math.exp(self._root_layer._nodes[0].log_val), True, evidence
    ...
    def deduce_p(self):
        ...
        WRITEME
        """
        scope_length = len(self._root_layer._nodes[0].var_scope)
        self.exact_eval([1]*scope_length)
        for node in self._input_layer._nodes:
            if math.exp(node.log_val) < 0.001:
                node.p =  3
            elif math.exp(node.log_val) < 0.01:
                node.p =  2
            elif math.exp(node.log_val) < 0.1:
                node.p =  1
            else:
                node.p = 0
    ...
    def get_variables_and_constraints1(self):
        """
            ...
            for jth_node in range(self._layers[l-ith_layer-1]._n_nodes):
                ...
                if node.node_type_str() == '*':
                    ...
                    for ith_child in range(len(node.children)):
                        w = node.weights[ith_child]
                        child = node.children[ith_child]
                        if child.node_type_str() == '+' or \
                           child.node_type_str() == '*' or \
                           child.node_type_str() == 'd':
                            equation += '{}{}*model.x[{}]'.format('+'*not_first,
                                                                  w,
                                                                  child.id)
                        else:
                            print("Child is neither a '+', a '*' or a 'd' ")
    ...
    def get_big_equation(self):
        return self._root_layer._nodes[0].get_big_equation()
    ...
    def get_variables_and_constraints(self, c):
        ...
        for node in self._input_layer._nodes:
            var_node.append(node.id)
            equation = "model.x[{}] <= ".format(node.id)
            equation += '{w0}*(1-model.l[{v}])+{w1}*model.l[{v}]'.format(w0 = (c**node.p) * math.exp(node._var_probs[0]),
                                                                         w1 = (c**node.p) * math.exp(node._var_probs[1]),
                                                                         v= node.var)
            constraints.append(equation)
    ...
    def get_LOG_variables_and_constraints(self):
        """
    function used to scroll through the SPN and get variables and constraints in order the compute the MPE thanks 
    to the nonlinear solver COUENNE and to the PYOMO library        
    """
        ...
        for ith_layer in range(l):
            for jth_node in range(self._layers[l-ith_layer-1]._n_nodes):
                ...
                node = self._layers[l-ith_layer-1]._nodes[jth_node]
                var_node.append(node.id)
                equation="log(exp(model.x[{}])) <= ".format(node.id)
                ...
                if node.node_type_str() == '*':
                    ...
                    for ith_child in range(len(node.children)):
                        ...
                        if child.node_type_str() == '+' or \
                           child.node_type_str() == '*' or \
                           child.node_type_str() == 'd':
                            equation += '{}{}exp({}+model.x[{}])'.format('log('*(not not_first),'+'*not_first, w, child.id)
                        else:
                            print("Child is neither a '+', a '*' or a 'd' ")
                        not_first = True
                    equation +=')'
        ...
        for node in self._input_layer._nodes:
            var_node.append(node.id)
            equation = "log(exp(model.x[{}])) <= ".format(node.id)
            equation += '{w0}*(1-model.l[{v}])+{w1}*model.l[{v}]'.format(w0 = node._var_probs[0],
                                                                         w1 = node._var_probs[1],
                                                                         v= node.var)
            constraints.append(equation)
    ...
    def MPE_poon_eval(self):
        track_memory = {}

        for node in self._input_layer._nodes:
            i = numpy.argmax(node._var_probs)
            node.log_val = node._var_probs[i]
            track_memory[node.id] = (node.var, i)

        l = len(self._layers) 
        for ith_layer in range(l):
            for node in self._layers[ith_layer]._nodes:

                if node.node_type_str() == '*':
                    track_memory[node.id] = []
                    node.log_val = 0
                    for child in node.children:
                        node.log_val += child.log_val
                        track_memory[node.id].append(child.id)


                elif node.node_type_str() == '+':
                    node.log_val = LOG_ZERO
                    for ith_child in range(len(node.children)):
                        w = node.weights[ith_child]
                        child = node.children[ith_child]
                        if log(w)+child.log_val > node.log_val:
                            node.log_val = log(w)+child.log_val
                            track_memory[node.id] = [child.id]

        root_id = self._root_layer._nodes[0]
        l = [self._root_layer._nodes[0].id]

        evidence_dic = {}
        while l:
            node_id = l.pop()
            memo = track_memory[node_id]
            if type(memo) == list:
                l = l + memo
            elif type(memo) == tuple:
                evidence_dic[memo[0]]=memo[1]
